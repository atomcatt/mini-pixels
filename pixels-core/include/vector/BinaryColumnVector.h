 ...
 * field is explicitly present, as opposed to provided by a dictionary
 * reference. In some cases, all the values will be in the same byte array to
 * begin with, but this need not be the case. If each value is in a separate
 * byte array to start with, or not all of the values are in the same original
 ...
 * first. You can mix "by value" and "by reference" in the same column vector,
...
public:
   ...
   */
  explicit BinaryColumnVector(uint64_t len = VectorizedRowBatch::DEFAULT_SIZE,
                              bool encoding = false);
