...
void BinaryColumnVector::setRef(int elementNum, uint8_t *const &sourceBuf,
...
void BinaryColumnVector::setVal(int elementNum, uint8_t *sourceBuf, int start,
                                int length) {
  if (elementNum >= writeIndex) {
    writeIndex = elementNum + 1;
  }
  if (buffer == nullptr) {
    // Issue #367: Lazy allocation only when necessary (i.e., setVal is used
    // instead of setRef).
    initBuffer(0);
  ...
  this->vector[elementNum] =
      duckdb::string_t((char *)(buffer + nextFree), length);
